...
type testTypedString struct {
	suite.Suite
}

func (t *testTypedString) TestParse() {
	cases := []struct {
		name     string
		s        string
		expected string
	...
	}{
		{
			name:     "valid",
			s:        "findme~showme",
			expected: "findme~showme",
		},
		{
			name: "empty raw string",
			s:    "~showme",
			err:  "invalid TypedString format",
		},
		{
			name: "empty raw string #1",
			s:    "   ~showme",
		...
		},
		{
			name: "invalid type",
			s:    "findme~sho;wme",
			err:  "invalid char found in Type",
		},
	}
...
func (t *testTypedString) TestEncodeJSON() {
	ht, err := ParseTypedString("showme~findme")
	t.NoError(err)
...
func (t *testTypedString) TestEncodeBSON() {
	ht, err := ParseTypedString("showme~findme")
	t.NoError(err)
...
func TestTypedString(t *testing.T) {
	suite.Run(t, new(testTypedString))
}
